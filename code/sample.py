...
def add_token(model, z, tokens, top_p, current_time, debug=False,temperature=1.0):
...
def generate(model, start_time, end_time, inputs=None, controls=None, top_p=1.0, debug=False, delta=DELTA*TIME_RESOLUTION,temperature=1.0):
    if inputs is None:
        inputs = []
    ...
    with tqdm(range(end_time-start_time)) as progress:
        ...
        while True:
            ...
            new_token = add_token(model, z, tokens, top_p, max(start_time,current_time),temperature=temperature)
...
def generate_ar(model, start_time, end_time, inputs=None, controls=None, top_p=1.0, debug=False, delta=DELTA*TIME_RESOLUTION,temperature=1.0):
    if inputs is None:
        inputs = []
